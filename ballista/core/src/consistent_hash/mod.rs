    ...
    N: Node,
{
        ...
        self.get_position_key(key, tolerance)
            .and_then(move |position_key| {
                if let Some(node_name) = self.virtual_nodes.get(&position_key) {
                    Some(&(self.node_replicas.get(node_name).unwrap().0))
                } else {
                    None
                }
            })
    ...
    fn get_position_key(&self, key: &[u8], tolerance: usize) -> Option<Vec<u8>> {
        ...
        while let Some((position_key, node_name)) = iter.next() {
...
mod test {
    ...
    #[test]
    fn test_tolerance() {
        let (mut consistent_hash, mut nodes, keys) = prepare_consistent_hash();
        ...
        for i in 0..nodes.len() {
            if i != 2 && i != 1 {
                let (mut node, num_replicas) =
                    consistent_hash.remove(nodes[i].name()).unwrap();
                node.available = false;
                consistent_hash.add(node, num_replicas);
            }
        }
