...
pub trait EventAction<E>: Send + Sync {
    ...
    async fn on_receive(
        ...
        rx_event: &mut mpsc::UnboundedReceiver<E>,
    ) -> Result<()>;
...
#[derive(Clone)]
pub struct UnboundedEventLoop<E> {
    pub name: String,
...
impl<E: Send + 'static> UnboundedEventLoop<E> {
    pub fn new(name: String, action: Arc<dyn EventAction<E>>) -> Self {
        Self {
            name,
    ...
    pub fn get_sender(&self) -> Result<UnboundedEventSender<E>> {
        Ok(UnboundedEventSender {
...
#[derive(Clone)]
pub struct UnboundedEventSender<E> {
    tx_event: mpsc::UnboundedSender<E>,
}

impl<E> UnboundedEventSender<E> {
