...
use crate::state::executor_manager::{
    coalesce_task_slots, ExecutorManager, ExecutorReservation,
};
...
impl<T: 'static + AsLogicalPlan, U: 'static + AsExecutionPlan> TaskManager<T, U> {
    ...
    pub async fn fill_reservations(
        ...
        let mut free_reservations: Vec<ExecutorReservation> = coalesced_reservations
            .into_iter()
            .map(|(executor_id, n_slots)| {
                ExecutorReservation::new_with_n(executor_id, n_slots)
            })
            .collect();
